...
#include "stl/Math/PhysicalDimension.h"
#include "stl/Math/ValueScaleTempl.h"
...
namespace AE::Math
{
	...
	public:
		...
		ND_ constexpr auto  operator /  (const PhysicalQuantity<Value_t,D,S> &rhs) const
		{
			using Scale = ValueScaleTempl::Div< Scale_t, S >; 
...
//-----------------------------------------------------------------------------
	...
	struct DefaultPhysicalQuantity
	{
		...
		using Ampers					= PhysicalQuantity< T, Dim::Amper >;					// A
		...
		using _GConstDim				= Dim::Meter::template Pow<2>::template Div< Dim::Kilogram::template Mul< Dim::Second::template Pow<2> >>;
