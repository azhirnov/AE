...
extern void UnitTest_CT_Counter ()
{
	...
	using counter1 = CT_Counter<uint>;
	using counter2 = CT_Counter<uint>;

	STATIC_ASSERT( counter1::Value() == 0 );
	STATIC_ASSERT( counter1::Value() == 0 );
	STATIC_ASSERT( counter1::Next() == 1 );
	STATIC_ASSERT( counter1::Next() == 2 );
	STATIC_ASSERT( counter1::Value() == 2 );
	
	STATIC_ASSERT( counter2::Next() == 1 );
